...
export function buildTasksMarkdown(screenManifest, figmaConfigured, figmaAutomationReady) {
  ...
  return [
    ...
    '- [ ] Keep each foundation guide decision-oriented and concise: target a 5-10 minute read with recommendation, rationale, usage boundaries, and exceptions',
...
function buildDesignTokenUsageMdx() {
  return [
    ...
    '  <section style={{ gridColumn: \'span 7\', padding: 20, borderRadius: 24, background: \'#eef2ff\', border: \'1px solid #a5b4fc\' }}>',
    '    <h3 style={{ marginTop: 0 }}>Minimum Deliverable</h3>',
    '    <p style={{ marginBottom: 0 }}>Show one observed value, one normalized token, one component usage example, and one migration or deprecation note. That is enough to keep handoff operational.</p>',
    '  </section>',
    '  <section style={{ gridColumn: \'span 5\', padding: 20, borderRadius: 24, background: \'#fff7ed\', border: \'1px solid #fdba74\' }}>',
    '    <h3 style={{ marginTop: 0 }}>Do Not Over-Document</h3>',
    '    <p style={{ marginBottom: 0 }}>Do not duplicate the entire token JSON in Storybook. Teach the meaning and the path to implementation instead.</p>',
    '  </section>',
    '</div>'
  ].join('\n')
}
...
function buildComponentStoryTemplate() {
  return [
    ...
    '    docs: {',
    '      description: {',
    "        component: 'Explain what this component is for, where it should be used, which foundation rules matter most, and any important state or accessibility caveats.'",
    '      }',
    '    }',
    '  },',
    '  argTypes: {',
