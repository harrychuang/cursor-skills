...
test('foundation paths include storybook docs templates', () => {
  ...
  assert.equal(paths.includes('src/stories/foundations/overview.mdx'), true)
  assert.equal(paths.includes('src/stories/foundations/guides.mdx'), true)
})
...
test('foundation entries include storybook docs templates', () => {
  ...
  const storybookGuidesEntry = entries.find(entry => entry.relativePath === 'src/stories/foundations/guides.mdx')
  const storybookReadmeEntry = entries.find(entry => entry.relativePath === 'src/stories/foundations/README.md')

  assert.ok(templateEntry)
  assert.ok(guidesEntry)
  assert.ok(storybookGuidesEntry)
  assert.ok(storybookReadmeEntry)
  assert.match(guidesEntry.content, /Foundations`, `Styles`, and `Components`/)
  assert.match(storybookGuidesEntry.content, /Foundations`, `Styles`, and `Components`/)
  assert.match(storybookReadmeEntry.content, /default Storybook setup can discover/)
