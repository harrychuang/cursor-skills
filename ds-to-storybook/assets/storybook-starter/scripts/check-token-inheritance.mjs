...
const tokenPattern = /(--ds-(ref|sys|comp)-[a-z0-9-]+)\s*:\s*([^;]+);/gi;
const referencePattern = /var\((--ds-(ref|sys|comp)-[a-z0-9-]+)\)/gi;
const violations = [];
...
for (const file of files) {
  ...
  for (const match of tokens) {
    ...
    if (declaredLayer !== file.layer) {
      violations.push(`${file.path}: ${tokenName} is declared in the wrong layer file.`);
    }

    const expectedReferenceLayer = allowedReferenceLayer(file.layer);
    const references = Array.from(value.matchAll(referencePattern), (reference) => reference[2]);
    ...
    if (expectedReferenceLayer) {
      for (const referenceLayer of references) {
        if (referenceLayer !== expectedReferenceLayer) {
          violations.push(
            `${file.path}: ${tokenName} references ${referenceLayer}; expected ${expectedReferenceLayer}.`,
          );
        }
      }
    }
...
console.log("Token inheritance check passed.");
