...
function getComponentSourceSlug(componentTitle: unknown): string {
...
function getFigmaNodeId(value: string): string | undefined {
...
function getDocumentedFigmaSourceUrl(
  ...
  const markdown =
    options.componentSpecModules[`../design-system/components/${sourceSlug}.md`];
  const nodeId =
    overrideNodeId ??
    markdown
      ?.split("\n")
  ...
  return `${options.designSystemFileUrl}?node-id=${nodeId.replace(":", "-")}`;
}
