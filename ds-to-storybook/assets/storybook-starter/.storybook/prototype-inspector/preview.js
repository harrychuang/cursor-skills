...
const flowLayoutStoragePrefix = "prototype-inspector:flow-layout";
const flowLayoutSchemaName = "storybook.prototype-flow-layout";
const flowLayoutSchemaVersion = 1;
const nodeWidth = 423;
...
const defaultNodeHeight = defaultNodePreviewHeight + nodeHeaderHeight;
...
function getFlowLayoutStorageKey(prototype) {
  return `${flowLayoutStoragePrefix}:${prototype.id ?? "prototype"}`;
}
...
function normalizeFlowLayoutPositions(value, nodeIds) {
  if (!isRecord(value)) {
    return {};
  }
  ...
  Object.entries(positionsSource).forEach(([routeId, position]) => {
    if (
      nodeIds.has(routeId) &&
      isRecord(position) &&
...
function readStoredFlowLayoutPositions(storageKey, nodeIds) {
  ...
  try {
    ...
    return storedValue
      ? normalizeFlowLayoutPositions(JSON.parse(storedValue), nodeIds)
      : {};
  } catch (error) {
...
function createFlowLayoutPayload(prototype, positions) {
  return {
    exportedAt: new Date().toISOString(),
    positions,
    prototypeId: prototype.id ?? null,
    schema: flowLayoutSchemaName,
    version: flowLayoutSchemaVersion,
  };
}
...
function writeStoredFlowLayout(storageKey, prototype, positions, nodeIds) {
  ...
  const normalizedPositions = normalizeFlowLayoutPositions({ positions }, nodeIds);
  ...
  try {
    ...
    window.localStorage.setItem(
      storageKey,
      JSON.stringify(createFlowLayoutPayload(prototype, normalizedPositions)),
    );
  } catch (error) {
...
function getDefaultNodePreviewHeight() {
  if (typeof document === "undefined") {
    return defaultNodePreviewHeight;
  }

  const tokenValue = getComputedStyle(document.documentElement)
    .getPropertyValue("--cm-sys-size-viewport-compact-height")
    .trim();
...
function getMeasuredFramePreviewHeight(iframe) {
  ...
  const routePreview = doc.querySelector('[data-component="inventory-prototype"]');

  if (routePreview) {
    const rect = routePreview.getBoundingClientRect();

    return Math.ceil(Math.max(rect.height, routePreview.scrollHeight));
  }
...
function PrototypeRouteCard({
  ...
  const measurePreviewHeight = (iframe) => {
    const measuredHeight = getMeasuredFramePreviewHeight(iframe);

    if (measuredHeight) {
      onPreviewHeightChange(route.id, measuredHeight);
      return true;
    }
  ...
  useEffect(() => {
    ...
    const updateMeasuredHeight = () => {
      ...
      if (iframe && measurePreviewHeight(iframe)) {
        return;
      }

      if (retryCount > 0) {
        retryCount -= 1;
        retryTimer = window.setTimeout(updateMeasuredHeight, 250);
      }
    };
  ...
  }, [onPreviewHeightChange, previewSource, route.id]);
  ...
  return createElement(
    "article",
    {
      ...
      style: {
        ...
        "--prototype-flow-card-width": `${nodeWidth}px`,
...
function PrototypeFlow({ prototype }) {
  ...
  const [routePreviewHeights, setRoutePreviewHeights] = useState({});
  ...
  const routePositionMap = useMemo(() => {
    ...
    routes.forEach((route, index) => {
      ...
      const previewHeight = routePreviewHeights[route.id] ?? defaultPreviewHeight;

      map.set(route.id, {
        ...position,
        height: previewHeight + nodeHeaderHeight,
        nodeType: "route",
        previewHeight,
        width: nodeWidth,
      });
  ...
  const handlePreviewHeightChange = useCallback((routeId, previewHeight) => {
    setRoutePreviewHeights((heights) => {
      const currentHeight = heights[routeId];

      if (currentHeight && Math.abs(currentHeight - previewHeight) < 2) {
        return heights;
      }

      return {
        ...heights,
        [routeId]: previewHeight,
      };
    });
  }, []);
  useEffect(() => {
  ...
  const exportLayout = () => {
    const payload = createFlowLayoutPayload(
      prototype,
  ...
  const importLayout = (event) => {
    ...
    reader.addEventListener("load", () => {
      try {
        ...
        const importedPositions = normalizeFlowLayoutPositions(
          importedLayout,
          canvasNodeIds,
        );
        ...
        createElement(
            ...
            renderedEdges.map((edge) =>
              createElement(
                "polyline",
                {
                  ...
                  key: edge.key,
                  markerEnd: "url(#prototype-inspector-arrow)",
          ...
          labeledEdges.map((edge) =>
            createElement(
              ...
              createElement("span", null, getEdgeLabelPrefix(edge)),
              createElement("strong", null, edge.labels.join(" / ")),
            ),
          ),
